...
BMP::BMPViewer::BMPViewer() = default;
BMP::BMPViewer::~BMPViewer() = default;

void BMP::BMPViewer::read(const std::string &path) {
...
void BMP::BMPViewer::drawLine(int x1, int y1, int x2, int y2) {
    int dx = abs(x2 - x1);
    int dy = abs(y2 - y1);
    int sx = (x1 < x2) ? 1 : -1;
    int sy = (y1 < y2) ? 1 : -1;
...
void BMP::BMPViewer::save(const std::string &outputPath) {
...
std::string BMP::BMPViewer::getOutputBMPFileName() {
    std::string path;
